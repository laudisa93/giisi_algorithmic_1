...
int busquedaLineal(int num,int v[TAM],int indice);
int busquedaBinaria(int num,int v[TAM],int inicio,int fin);
...
int main()
{
    ...
    inicio = GetTickCount();
    for(i=0;i<divisor;i++){
        pos=busquedaBinaria(numBusca,v,0,TAM);//pos=busquedaLineal(numBusca,v,0);
    }
    final = GetTickCount();
...
int busquedaLineal(int num,int v[TAM],int indice){
    ...
    if(indice<=TAM){
        if(num==v[indice]){
            ...
            encontrado=busquedaLineal(num,v,indice+1);
        }
    }
    return encontrado;
}
...
int busquedaBinaria(int num, int v[TAM],int inicio,int fin){
    ...
    if(inicio<=fin){
        ...
        if(num==v[medio]){
            ...
            encontrado=busquedaBinaria(num, v,inicio,medio-1);
        }else{
            encontrado=busquedaBinaria(num, v,medio+1,fin);
        }
    }
    return encontrado;
}
